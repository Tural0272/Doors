...
const WorksPhotoResults = ({
  ...
  const [searchDoors, setSearchDoors] = useState(images);
  const [displayedDoors, setDisplayedDoors] = useState(images);
            ...
            <TableHead>
              <TableRow>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedDoorIds.length === images.length}
                    color="primary"
                    indeterminate={
                      selectedDoorIds.length > 0 &&
                      selectedDoorIds.length < images.length
                    }
...
WorksPhotoResults.propTypes = {
  images: PropTypes.array.isRequired,
};

export default WorksPhotoResults;
